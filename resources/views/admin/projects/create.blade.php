...
<div class="max-w-4xl mx-auto space-y-6">
    <div>
        <h1 class="text-2xl font-bold tracking-tight text-white">Tambah Project Baru</h1>
        <p class="text-sm text-slate-400 mt-1">Masukkan detail karya atau aplikasi buatan member kelas.</p>
    </div>
        ...
        <form action="{{ route('projects.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full bg-slate-950 border @error('name') border-rose-500 @else border-slate-800 @enderror rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-indigo-500">
                    @error('name')
                ...
                <!-- Thumbnail -->
                <div class="space-y-2">
                    <label for="thumbnail" class="text-sm font-semibold text-slate-200">Thumbnail Image (Max 2MB)</label>
                ...
                <!-- Repository URL -->
                <div class="space-y-2">
                    <label for="repository_url" class="text-sm font-semibold text-slate-200">Repository URL (GitHub, GitLab, dll)</label>
                    <input type="url" name="repository_url" id="repository_url" value="{{ old('repository_url') }}" placeholder="https://github.com/..." class="w-full bg-slate-950 border @error('repository_url') border-rose-500 @else border-slate-800 @enderror rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-indigo-500">
                ...
                <div class="space-y-2">
                    ...
                    <input type="url" name="demo_url" id="demo_url" value="{{ old('demo_url') }}" placeholder="https://..." class="w-full bg-slate-950 border @error('demo_url') border-rose-500 @else border-slate-800 @enderror rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-indigo-500">
                    @error('demo_url')
                        <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="4" placeholder="Jelaskan kegunaan, teknologi yang dipakai, dll..." class="w-full bg-slate-950 border @error('description') border-rose-500 @else border-slate-800 @enderror rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-indigo-500">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                @enderror
            </div>
